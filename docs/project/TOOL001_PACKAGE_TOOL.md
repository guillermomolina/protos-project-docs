...
| TOOL001-F2D3B | exact workspace package-backed module resolver | IN_PROGRESS | TOOL001-F2D3B1A/F2D3B1B1 published | Key identity + project-root/package index CLOSED; B1B2 exact member binding READY; routing remains dependency-gated. |
| TOOL001-F2D3B1 | package identity + source mechanism | IN_PROGRESS | TOOL001-F2D3B1A/F2D3B1B1 published | Canonical key and project-root/package index CLOSED; exact member/source binding remains. |
...
| TOOL001-F2D3B1B | physical source mechanism | IN_PROGRESS | TOOL001-F2D3B1B1 published | Split into root/index, exact member-directory binding, and logical-module source binding. |
...
| TOOL001-F2D3B1B2 | exact member-location directory binding | IN_PROGRESS | TOOL001-F2D3B1B2A published | Split into exact direct-child lookup, confined canonical location traversal, then immutable package-directory binding. |
...
| TOOL001-F2D3B1B2B | confined canonical member-location traversal | READY | — | Traverse one already-canonical `/`-separated member location component-by-component and enforce real-path confinement/symlink behavior. |
| TOOL001-F2D3B1B2C | immutable package -> physical-directory binding | BLOCKED_BY_DEPENDENCIES | TOOL001-F2D3B1B2B | Apply the closed traversal to the detached B1B1 package index and produce exact immutable package-directory bindings. |
| TOOL001-F2D3B1B3 | logical module -> exact regular `.protos` source | BLOCKED_BY_DEPENDENCIES | TOOL001-F2D3B1B2C | Map portable logical segments beneath an already-bound package root to one exact-case regular confined source file. |
| TOOL001-F2D3B2 | resolver routing | BLOCKED_BY_DEPENDENCIES | TOOL001-F2D3B1 | Routing begins only after key + source mechanism closes. |
| TOOL001-F2D3B2A | self: routing | BLOCKED_BY_DEPENDENCIES | TOOL001-F2D3B1B3 | First observable package import routing slice after complete key + source mechanism; prefer Protos-source behavior tests. |
| TOOL001-F2D3B2B | dep: edge/export routing | BLOCKED_BY_DEPENDENCIES | TOOL001-F2D3B2A | Follow exact detached edge then target export without bypass. |
