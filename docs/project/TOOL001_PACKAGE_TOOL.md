...
| TOOL001-F2D3B1B2 | exact member-location directory binding | READY | — | Bind each canonical non-root workspace location to an exact-case in-root physical directory; no module file lookup. |
| TOOL001-F2D3B1B3 | logical module -> exact regular `.protos` source | BLOCKED_BY_DEPENDENCIES | TOOL001-F2D3B1B2 | Map portable logical segments beneath an already-bound package root to one exact-case regular confined source file. |
