...
| TOOL001-F2E3 | external-node execution-plan construction | IN_PROGRESS | `ff2821f411721eba234b3d52d6606f9569fc5b0a`; D053/D056 RATIFIED | Generation-2 external-leaf construction is published; D056 now ratifies workspace-only `path` semantics for immutable consumption, so remaining F2E3 may read the same verified external ManifestV1, reject external path dependencies and validate registry/Git transitive edges mechanically. |
