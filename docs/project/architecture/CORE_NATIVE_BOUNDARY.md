...
requires the existing state-bearing ordinary-object representation, and delegates directly to
`ProtosObjectValue.freeze()`. That existing primitive remains the single owner of structural state:
OPEN and CLOSED become FROZEN, FROZEN remains idempotently FROZEN, and every successful call
returns the exact original receiver. Freezing is shallow and does not recursively freeze values
reachable through slots or collection elements.

Retained guest conformance proves the state boundary rather than only Java mechanism: freezing an
ordinary object forbids local-slot creation, existing-slot assignment and local removal; freezing a
previously CLOSED object hardens its formerly-assignable value state; the already-frozen standard
root remains idempotent; an Array rejects indexed replacement after freeze; and a Map rejects keyed
replacement after freeze. Reachable child objects remain independently mutable, preserving the
normative shallow rule. Incorrect arity signals through the existing Object error path.

This requires exactly one new reviewed native Closure construction site in the existing
`ProtosStandardObjectProtocol` provider. The audited Core boundary advances from **139 sites /
36 providers** to **140 sites / 36 providers**, with `ProtosStandardObjectProtocol.java`
advancing from 12 to 13 sites. No specification, syntax, source-style exception, new runtime family,
deep-freeze mechanism, transfer/ownership rule, or second structural-state model is introduced.
...
existing standard Object representation bridge. Each call validates zero positional arguments,
observes only the receiver's own local ordinary-object slot table, snapshots those names, orders
String names lexicographically by Unicode scalar-value sequence, and materializes a fresh open
standard Array containing fresh semantic String values. Inherited/delegated slots are excluded;
opaque represented Core values have no representation-owned ordinary local-slot table and
therefore produce the same fresh empty standard Array shape.

This requires exactly one new reviewed native Closure construction site in the existing
`ProtosStandardObjectProtocol` provider. The audited Core boundary advances from **136 sites /
36 providers** to **137 sites / 36 providers**, with `ProtosStandardObjectProtocol.java`
advancing from 9 to 10 sites. Retained ordinary-Protos conformance covers local-only reflection,
prefix and BMP-versus-astral Unicode-scalar ordering, fresh independent mutable snapshots,
empty-result freshness/standard-Array parentage, represented-value empty observation, and arity
rejection. The conformance sources use idiomatic bracket indexing under the approved source-style
policy; no source-style exception, specification change, new runtime family, or delegated
reflection model is introduced.
...
I028-D3 adds exactly one shared native `accept` Closure to the existing hidden TcpListener protocol provider. Each call creates one independent `ProtosIoOperation` on the listener's D2 lifecycle; no semantic accept FIFO, one-pending restriction, owner-thread affinity or backend scheduling identity is introduced. Backend success crosses commitment once, materializes one fresh standard TcpConnection only after validating both logical endpoint snapshots through the existing D048 recognition bridge, and explicitly releases late, duplicate, cancelled or invalid descriptors. Close cutover and Actor termination reuse the same lifecycle/cancellation machinery. The audited Core boundary advances from **133 sites / 35 providers** to **134 sites / 35 providers**.
...
| `ProtosStandardNetworkProtocol.java` | 2 | resource/capability bridge | D047/PLAT002/PLAT003 `connectTcp` and `listenTcp` validate represented Network authority and their exact portable request domains before authority exercise, then cross host-neutral asynchronous acquisition/commitment/late-custody boundaries without selecting socket/reactor/backend identity. |
...
| `ProtosStandardTcpListenerProtocol.java` | 3 | resource/capability bridge | D052 listener `accept`, `localPort` and Closable `close` validate actual operational family receivers. `accept` owns independent Future/commitment/cancellation/late-custody handoff through the shared listener lifecycle and materializes recognized-endpoint TcpConnection descriptors without selecting backend identity; observation and close retain the D2 contracts. |
