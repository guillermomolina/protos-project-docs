...
| I026-A4B3 | IN_PROGRESS | — | I026-A4B2 | Production-driver cutover is partially published: ordinary CLI/REPL entry uses Process-scoped PLAT001 hosting in `0.2.287-SNAPSHOT`. Bundled-tool + exact/fresh/captured/workspace execution is the next mechanical phase under Issue #89, followed by module/initial-module routing and final direct-entry retirement. No new durable sub-item is allocated for these execution phases. |
...
authority. D049 resolves B010's independent standard-root structural-state/isolation question. A4B2B3 is now CLOSED: A publishes atomic frozen Core construction and complete shared-standard graph sealing, while B publishes the already-approved A+ `ProtosLanguageContext`-local executable projection with concurrent multi-Process no-cross-CallTarget evidence. B010, A4B2B and A4B2 are CLOSED. A4B3 is IN_PROGRESS after publishing the ordinary CLI/REPL Process-context cutover in `0.2.287-SNAPSHOT`; bundled-tool + exact/fresh/captured/workspace execution is next, while module/initial-module routing and final legacy-entry retirement remain later mechanical phases under the same I026-A4B3 work item. I026-D remains independently READY.
