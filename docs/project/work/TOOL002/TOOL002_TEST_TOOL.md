...
| TOOL002-I | READY | D076 Candidate D, D077 Candidate D, D087 Candidate B, D091 Candidate A′, D094 Candidate A′ refined, D097 Candidate B′ and D098 Candidate A′ are RATIFIED. I1-I6D are CLOSED; I6E remains the next executable slice to acquire the optional exact corpus-root sibling `resource-requirements.toml` through the existing corpus Filesystem, distinguish true absence from present non-regular/read/parse/join failure, then feed I6C + I6D before filtering/sharding. D097 fixes catalog v1 `scope` to `placement` or `run` strictly as the capacity accounting/reservation domain; D098 now fixes one explicit logical `provider` plus optional provider-scoped logical `profile`, both canonical inert identities, with no key inference, implementation reference, arbitrary inline provider config or persisted authority. Capability lifetime, physical topology, provider API/loading/configuration, cross-run/global coordination, public requirements overrides/search/merge, catalog source/CLI, public `std:toml`, a general metadata namespace and other deferred resource contracts remain unselected. |
