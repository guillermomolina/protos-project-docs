...
| TOOL002-I | READY | D076 Candidate D, D077 Candidate D, D087 Candidate B, D091 Candidate A′ and D094 Candidate A′ refined are RATIFIED. I1-I6D are CLOSED; I6E is READY to acquire the optional exact corpus-root sibling `resource-requirements.toml` through the existing corpus Filesystem, distinguish true absence from present non-regular/read/parse/join failure, then feed I6C + I6D before filtering/sharding. Public requirements overrides/search/merge, catalog CLI/source, exact scope vocabulary, provider APIs, public `std:toml`, a general metadata namespace and other deferred resource contracts remain unselected. |
