...
| TOOL002-I | IN_PROGRESS | D076 Candidate D, D077 Candidate D, D087 Candidate B, D091 Candidate A′, D094 Candidate A′ refined, D097 Candidate B′, D098 Candidate A′, D099 Candidate A′ refined, D101 Candidate C′ and D105 Candidate E′ are RATIFIED. I1-I7A, I7B1, I7B2, I7B3A, I7B3B, I8A and I8B are CLOSED. D105 fixes composition of I8B with the existing H scheduler: each immutable logical H window contains the next `min(jobs, remainingCases)` CaseSpecs; within that window stable TestPlan-order resource rounds may bypass temporarily blocked members only with later feasible members of the same window; no later window may advance until the current window reaches terminal completion; each round retains reservations through terminal cleanup and releases them before rescanning remaining window members. Resource-free windows remain exactly one H round. D105 introduces no global eager backfill, priority, aging, fair-share, preemption, placement topology or provider policy. The next bounded executable slice may compose I8A/I8B with the H Runner under this contract; provider resolution/provisioning and attempt-private capability delivery remain later D076 boundaries. |
