...
| TOOL002-I | IN_PROGRESS | D076 Candidate D, D077 Candidate D, D087 Candidate B, D091 Candidate A′, D094 Candidate A′ refined, D097 Candidate B′, D098 Candidate A′, D099 Candidate A′ refined, D101 Candidate C′, D105 Candidate E′, D107 Candidate D′ and D108 Candidate C′ are RATIFIED. I1-I7A, I7B1, I7B2, I7B3A, I7B3B, I8A, I8B, I8C, I8D1, I8D2, I8D3 and I8D4A are CLOSED. I8D4A establishes the private D108 terminal-attempt foundation without Runner/Main wiring: `ProtosProcessRuntime` exposes a host-only wait for actual `TERMINATED` state using its existing terminal `notifyAll`; `ProtosProcessExecutionHost` gains a default terminal-disposition wait and the Polyglot host overrides it with an exact Context-close success/failure barrier distinct from `closeRequested`. `ProtosTestResourceAttemptCompletion` is the inert dual-lane carrier with optional guest observation, ordered infrastructure failures and SAFE/UNSAFE capacity disposition. `ProtosTestResourceAttemptTerminalizer` preserves guest evidence, waits semantic Process termination and execution-host terminal disposition, then awaits D107 ProviderLease cleanup; host/cleanup failure is infrastructure evidence and UNSAFE capacity, never guest Error. Cleanup still runs after execution-host failure. I8D4A does not yet handle pre-child provisioning failure, install a resourceful async Protos bridge, release I8B reservations, change Runner round policy, or wire public Main. I8D4B may compose I8D2 provisioning + I8D3 resources bootstrap + I8D4A terminal completion into one private resourceful async attempt bridge; I8D4C remains D108 round-drain/fail-stop Runner integration. |
