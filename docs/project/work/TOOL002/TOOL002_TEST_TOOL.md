...
| TOOL002-I | IN_PROGRESS | D076 Candidate D, D077 Candidate D, D087 Candidate B, D091 Candidate A′, D094 Candidate A′ refined, D097 Candidate B′, D098 Candidate A′ and D099 Candidate A′ refined are RATIFIED. I1-I6E are CLOSED. I7A adds the pure strict resource-catalog v1 TOML schema parser over already-acquired text: canonical resource key, positive capacity, `scope ∈ {placement, run}`, mandatory inert logical `provider`, optional provider-scoped inert `profile`, strict unknown-field/version rejection and duplicate-key fail-closed validation. D099 filesystem/CLI acquisition, D076 reservation/capability provisioning, provider implementation/loading, Main wiring, physical topology, cross-run coordination and public `std:toml` remain outside I7A. |
