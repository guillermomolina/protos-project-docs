...
| TOOL002-I | IN_PROGRESS | D076 Candidate D, D077 Candidate D, D087 Candidate B, D091 Candidate A′, D094 Candidate A′ refined, D097 Candidate B′, D098 Candidate A′, D099 Candidate A′ refined, D101 Candidate C′, D105 Candidate E′, D107 Candidate D′ and D108 Candidate C′ are RATIFIED. I1-I7A, I7B1, I7B2, I7B3A, I7B3B, I8A, I8B, I8C, I8D1, I8D2, I8D3, I8D4A, I8D4B, I8D4C1, I8D4C2 and I8D4C3 are CLOSED. I8D4C3 adds a new private `runD108WithResourcesWithLoader` Runner path without mutating the historical I8C entrypoint. Resource-free cases retain the H fast path; non-empty bound sets are projected mechanically to the exact C1/C2 six-field inert provider snapshot and routed through resourceful ordinary/live-inspection callbacks. Each admitted round is fully drained before disposition. I8B reservations are released only when the terminal completion reports `capacitySafe`; UNSAFE reservations remain held. Guest observations already produced are preserved in TestPlan order even when later cleanup fails; attempts with no guest observation fabricate no CaseRun. Any infrastructure outcome records the owning CaseSpec plus the complete D108 completion, prevents current-window rescan, prevents later fixed-H windows including resource-free cases, and records exact selected CaseSpecs never admitted by cutover. The private D108 run envelope preserves ordered guest CaseRuns, ordinary unsupported count, per-attempt infrastructure evidence, cutover-not-admitted CaseSpecs and retained-reservation count. Only a completely healthy envelope may project back to the historical `runTuple`. This is not final CLI/reporting policy: Main remains unchanged and production provider-registry wiring plus final infrastructure-abort reporting remain later integration boundaries. |
