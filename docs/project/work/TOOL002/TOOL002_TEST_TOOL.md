...
| TOOL002-I | IN_PROGRESS | D076 Candidate D, D077 Candidate D, D087 Candidate B, D091 Candidate A′, D094 Candidate A′ refined, D097 Candidate B′, D098 Candidate A′, D099 Candidate A′ refined, D101 Candidate C′, D105 Candidate E′, D107 Candidate D′, D108 Candidate C′, D113 Candidate B′ and D114 Candidate B′ are RATIFIED. I1-I7A, I7B1, I7B2, I7B3A, I7B3B, I8A, I8B, I8C, I8D1, I8D2, I8D3, I8D4A, I8D4B, I8D4C1, I8D4C2, I8D4C3 and I8D5A are CLOSED. I8D5A materializes only the D113 production host-bootstrap boundary: add one public host facade in `execution` that owns the already-private C1/C2 facilities and installs one explicit immutable empty provider registry as the valid/default local environment. `ProtosTestToolAsyncExecutionScope` owns/closes that facade with the existing PLAT023 Submission, so production Test Tool sessions now receive the `resourceExecutionAsync` and `resourceExecutionInspectAsync` bootstrap routes without making ProviderAdapter, ProviderLease, provider configuration, credentials or registry mutation public. Unknown logical providers fail closed through the existing D107/D108 infrastructure lane before guest Process creation and clean no-lease rollback remains capacity SAFE. Main/Runner public selection is unchanged in I8D5A, so resource-aware scheduling/reporting is not activated yet. I8D5B is READY to project the private C3 envelope to the ratified D114 TestRunOutcome without changing CLI exit behavior; I8D5C remains the later atomic Main/CLI cutover. |
