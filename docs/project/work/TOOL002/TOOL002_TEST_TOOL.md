...
| TOOL002-I | IN_PROGRESS | D076 Candidate D, D077 Candidate D, D087 Candidate B, D091 Candidate A′, D094 Candidate A′ refined, D097 Candidate B′, D098 Candidate A′, D099 Candidate A′ refined, D101 Candidate C′, D105 Candidate E′ and D107 Candidate D′ are RATIFIED. I1-I7A, I7B1, I7B2, I7B3A, I7B3B, I8A, I8B, I8C, I8D1 and I8D2 are CLOSED. I8D2 adds the private host-side D107 multi-provider transaction coordinator over I8D1: already-reserved binding facts are snapshotted and grouped once per logical provider in stable first-appearance order; providers resolve only through the explicit registry and provision sequentially through async-capable adapter stages; every returned lease must cover exactly its provider group's required D077 keys before the next provider runs; duplicate resource keys fail before provider invocation. A later provisioning or coverage failure rolls back the current/previous successful leases in strict reverse acquisition order, attempting all cleanups and retaining cleanup failures as infrastructure evidence. Success returns a host-owned transaction with immutable candidate capability values restored to original requirement-key order and all leases retained for later terminal cleanup; transaction cleanup is also reverse-order and attempts all providers after a failure. Resource-free input performs zero registry/provider work. I8D2 still does not create the guest `resources` bundle, modify Process/RootActor bootstrap, wire Runner/Main, or release I8B reservations. The next bounded executable slice may project the successful transaction's candidate capability snapshot as one frozen RootActor-only `resources` grant while preserving host lease custody; Runner/Main integration remains later. |
