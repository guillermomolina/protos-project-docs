...
| TOOL002-I | IN_PROGRESS | D076 Candidate D, D077 Candidate D, D087 Candidate B, D091 Candidate A′, D094 Candidate A′ refined, D097 Candidate B′, D098 Candidate A′, D099 Candidate A′ refined, D101 Candidate C′, D105 Candidate E′ and D107 Candidate D′ are RATIFIED. I1-I7A, I7B1, I7B2, I7B3A, I7B3B, I8A, I8B and I8C are CLOSED. D107 fixes the remaining provider/capability architecture after I8C: exact environment registry resolution maps inert D098 provider identities to transport-neutral configured ProviderAdapters; one attempt groups already-reserved bindings per provider; provider groups provision transactionally before child creation; partial success unwinds in reverse acquisition order; successful host-owned ProviderLeases remain in custody through terminal child cleanup; only then does provider cleanup run, followed by I8B reservation release. The fresh RootActor receives only one frozen attempt-private `resources` capability bundle keyed by exact D077 resource key; provider instances, credentials, cleanup handles and registry authority never enter guest state. Resource-free attempts retain the zero-provider fast path and have no `resources` local. Unknown provider/profile, provisioning failure and cleanup failure remain infrastructure outcomes. Cleanup that cannot be confirmed safe must not silently return affected resource capacity; baseline behavior is fail closed rather than inventing quarantine/recovery. Public provider distribution/install format, concrete provider set, secret-store mechanism, physical worker topology and distributed reservation remain deferred. The next bounded executable work may implement the private D107 registry/transaction/lease/bootstrap foundation; public resourceful `Main.protos` wiring remains deferred until that foundation is proven. |
