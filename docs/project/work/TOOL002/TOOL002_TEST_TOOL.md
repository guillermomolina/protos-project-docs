...
| TOOL002-I | READY | D076 Candidate D, D077 Candidate D, D087 Candidate B, D091 Candidate A′, D094 Candidate A′ refined and D097 Candidate B′ are RATIFIED. I1-I6D are CLOSED; I6E remains the next executable slice to acquire the optional exact corpus-root sibling `resource-requirements.toml` through the existing corpus Filesystem, distinguish true absence from present non-regular/read/parse/join failure, then feed I6C + I6D before filtering/sharding. D097 fixes catalog v1 `scope` to `placement` or `run` strictly as the capacity accounting/reservation domain for later catalog work; capability lifetime, physical topology, provider ownership/externality and cross-run/global coordination stay orthogonal. Public requirements overrides/search/merge, catalog source/CLI, provider APIs, topology descriptors/selectors, public `std:toml`, a general metadata namespace and other deferred resource contracts remain unselected. |
