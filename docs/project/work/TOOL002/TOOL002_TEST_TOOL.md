...
| TOOL002-I | IN_PROGRESS | D076 Candidate D, D077 Candidate D, D087 Candidate B, D091 Candidate A′, D094 Candidate A′ refined, D097 Candidate B′, D098 Candidate A′, D099 Candidate A′ refined, D101 Candidate C′ and D105 Candidate E′ are RATIFIED. I1-I7A, I7B1, I7B2, I7B3A, I7B3B, I8A, I8B and I8C are CLOSED. I8C composes I8A/I8B with the H scheduler through private `Runner.runBoundedWithResources*` entrypoints under D105: each immutable H window contains at most `jobs` selected CaseSpecs; static bindings/source are materialized once for that window; each stable TestPlan-order resource round admits all currently complete-set reservable members, leaves contended members pending, waits at the H execution barrier, releases every round reservation, then retries only remaining members. Backfill is same-window only; results are buffered by immutable window index and emitted in TestPlan order. Resource-free windows retain one-round H behavior. Public `Main.protos` and I6E requirements-sidecar discovery remain intentionally unwired, because D076 provider resolution/provisioning and attempt-private capability delivery are still absent. Before public resourceful execution, route that provider/capability boundary through the normal decision gate rather than inventing a provider API inside Runner/Main. |
