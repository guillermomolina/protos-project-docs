...
| TOOL002-I | IN_PROGRESS | D076 Candidate D, D077 Candidate D, D087 Candidate B, D091 Candidate A′, D094 Candidate A′ refined, D097 Candidate B′, D098 Candidate A′, D099 Candidate A′ refined, D101 Candidate C′, D105 Candidate E′ and D107 Candidate D′ are RATIFIED. I1-I7A, I7B1, I7B2, I7B3A, I7B3B, I8A, I8B, I8C, I8D1, I8D2 and I8D3 are CLOSED. I8D3 implements only D107 RootActor capability projection: the successful host capability snapshot is converted to the ordinary Core `Map` through standard `atPut` message semantics using exact String keys, then frozen before bootstrap; a bootstrap-stable optional resources grant is retained by `ProtosProcessRuntime` alongside independent Filesystem/Network grants and `ProtosActorBootstrap` exposes it only as the RootActor initial local `resources` after validating frozen state and exact current-Prelude Map parent. Existing public standalone bootstrap signatures remain unchanged; a package-private resourceful bootstrap entry accepts the already-built frozen Map. Resource-free Process creation stores no resources grant and creates no `resources` local. I8D3 contains no provider registry/lease authority in guest state and does not connect I8D2 to Runner/Main or I8B release. The next bounded executable slice may compose I8C admission/reservation with I8D2 provisioning and I8D3 Process bootstrap while retaining provider-lease custody through terminal child cleanup; public Main/I6E wiring remains later. |
