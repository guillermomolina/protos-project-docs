...
| TOOL002-I | READY | D076 Candidate D, D077 Candidate D, D087 Candidate B, D091 Candidate A′, D094 Candidate A′ refined, D097 Candidate B′, D098 Candidate A′ and D099 Candidate A′ refined are RATIFIED. I1-I6D are CLOSED; I6E remains the next executable slice to acquire the optional exact corpus-root sibling `resource-requirements.toml` through the existing corpus Filesystem, distinguish true absence from present non-regular/read/parse/join failure, then feed I6C + I6D before filtering/sharding. D097 fixes catalog v1 `scope` to `placement` or `run` strictly as the capacity accounting/reservation domain; D098 now fixes one explicit logical `provider` plus optional provider-scoped logical `profile`, both canonical inert identities, with no key inference, implementation reference, arbitrary inline provider config or persisted authority. D099 now fixes catalog acquisition to optional single explicit local `--resource-catalog PATH`: zero/one occurrence, no discovery/env/merge/stdin/URI/corpus binding, fail-closed source acquisition, and no host-path durable or distributed identity. Capability lifetime, physical topology, provider API/loading/configuration, cross-run/global coordination, public requirements overrides/search/merge, public `std:toml`, a general metadata namespace and other deferred resource contracts remain unselected. |
