...
| TOOL002-I | CLOSED | D076 Candidate D, D077 Candidate D, D087 Candidate B, D091 Candidate A′, D094 Candidate A′ refined, D097 Candidate B′, D098 Candidate A′, D099 Candidate A′ refined, D101 Candidate C′, D105 Candidate E′, D107 Candidate D′, D108 Candidate C′, D113 Candidate B′, D114 Candidate B′ and D116 Candidate B′ are RATIFIED. I1-I7A, I7B1, I7B2, I7B3A, I7B3B, I8A, I8B, I8C, I8D1, I8D2, I8D3, I8D4A, I8D4B, I8D4C1, I8D4C2, I8D4C3, I8D5A, I8D5B and I8D5C are CLOSED. I8D5C completes the atomic public Main/CLI cutover. All four currently owned plans execute through the D108 resource-aware Runner path with plan-specific ordinary and resourceful execution/inspection routes bound to the correct Prelude. Guest/test failure never short-circuits later owned plans; D108 infrastructure abort does, and exact already-produced/cutover evidence is projected through D114. Bundled Protos aggregates all actually executed plan-local runTuples under D116, preserves the historical primary runTuple as the healthy compatibility payload, and selects final CLI classification 0/1/3. Java consumes only that selected classification, validates the carrier shape, and emits bounded sanitized infrastructure-abort counts without recomputing plan pass/fail policy. D113 remains an explicit immutable empty provider registry by default with no discovery. TOOL002-I resource-aware public execution/reporting integration is complete; I closes at the published I8D5C public cutover; remaining Test Tool closure work belongs to TOOL002-J. |
| TOOL002-J | CLOSED | Final Test Tool closure materializes the selected Java-first / Protos-tool-second validation pipeline without adding Test Tool semantics: repository Java/runtime validation remains stage one and the retained real public `protos test --jobs 2` CLI checkpoint is an explicit fail-closed second stage. A Tool-owned executable closure guard reconciles D108/D114/D116 retained ownership: bundled Protos still owns all four plan executions, N-plan aggregation and final 0/1/3 classification; Java only validates/consumes that carrier and emits bounded infrastructure diagnostics. The scheduler/distribution architecture remains backend-neutral with fresh semantic Process/RootActor isolation, bounded deterministic scheduling, explicit resource/capability ownership and separate guest-vs-infrastructure evidence. Machine-readable report schema, retries/flaky policy, hard OS-worker timeout/recovery, sharding/remote-worker protocol, watch/incremental mode, caching, `jobs=auto` and new provider/resource semantics remain deferred. J closes by reusing the immutable full Java/runtime evidence at `e9c560db` (1893 tests, 0 failures, 0 errors, Maven worktree integrity PASS), revalidating the exact executable drift chain `1a3dfe80` (LIB011-D) -> `04c8ba9e` (PLAT031 buffered-I/O convergence) -> `875b62c1` (LIB011-E) with the union of their bounded focal tests on the integrated state, rejecting any later non-neutral drift (allowing only docs/CHANGELOG and root-project-version-only POM drift), then running the J closure guard and the public Protos Test Tool checkpoint; no specification, runtime implementation, native-boundary or implementation-version change. |
