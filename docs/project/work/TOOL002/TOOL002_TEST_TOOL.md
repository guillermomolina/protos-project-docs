...
| TOOL002-I | READY | D076 Candidate D, D077 Candidate D and D087 Candidate B are RATIFIED. I1-I5 are CLOSED. The next bounded persistent-TOML slice may establish the private shared Protos-source bootstrap parser owner and bundled-tool-private resolution path, while Package Tool and Test Tool retain separate schema semantics and current persisted generations remain explicitly pinned to TOML 1.0. Public sidecar filename, catalog CLI spelling, exact scope vocabulary, provider APIs, a public `std:toml`, a general internal tool package graph and other D077/D087-deferred contracts remain unselected and must cross a later approval gate if reached. |
