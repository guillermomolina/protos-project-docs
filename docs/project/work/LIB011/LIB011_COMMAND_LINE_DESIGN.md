...
**CLOSED — recursive traversal published under D115 Candidate C′.**

`CommandLine.parse(spec, arguments)` now consumes the canonical frozen
`CommandSpec` tree directly and traverses only the command scopes selected by the
explicit argument stream. The temporary LIB011-B rejection of specs containing
subcommands and the B-stage whole-tree re-canonicalization are removed.

For each active scope, option and exact-child lookup Maps are invocation-local.
A direct child is selected at the earliest exact child-name token for which the
complete parent positional prefix is feasible: all required parent
`minOccurrences` can be satisfied and bounded parent maximum capacity has not
already been exceeded. The parent prefix is then materialized exactly once under
D111 B′. The selecting token is retained as the child `CommandResult.tokenIndex`
and is never duplicated as a parent positional occurrence.

A recognized value-taking option still consumes its value literally before child
recognition. Structural current-scope `--` disables both option and child
recognition for that scope. Once selected, child ownership of the remaining token
stream is irreversible; options declared only on the parent do not leak across
the boundary. Shared behavior continues to use ordinary explicit `OptionSpec`
composition at each accepting node.

Traversal recursively applies the same law at every depth. The parser does not
flatten or globally index the entire command tree and does not introduce typed
probing, callbacks, guest execution, environment/filesystem authority,
persistent/global options, fuzzy matching, parse scoring or backtracking.

The implementation target remains `O(T + Svisited)` time and
`O(result + Svisited)` memory, where `Svisited` is specification material for the
actually visited command path. Semantic authority is the Protos corpus under
`protos/tests/library/cli/**`; the Java class remains only the execution harness
and complementary structural evidence.

Publication remains impact-aware `FOCAL_BOUNDED`; broader integrated validation
is deferred to the owning top-level LIB011 closure.

The governing durable traversal decision is
