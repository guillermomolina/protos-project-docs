...
|---|---|---|---|---|
| AUD003 | Protos source-style conformance audit | CLOSED | `SAME_COMMIT` | A1-E complete. Final execution-time E15 rescan matches the exact reviewed B1/B2/C17 survivor inventories after E14 reconciled the two late pre-guard LM008-D4 Boolean-negation cases; top-level publication validation is green; active source-style documentation is reconciled; and the AUD003-D differential prevention gate remains active in publication validation/CI. No syntax or semantics were selected by the audit. See `docs/project/work/AUD003/AUD003_PROTOS_SOURCE_STYLE_CONFORMANCE_AUDIT.md`. |
...
| I031 | Standard Object reflection/mutation/state publication | CLOSED | A `d50ab69f`; B `6b5d029f`; C `c9677779`; D `42f6e0b9`; E `SAME_COMMIT` | A-D publish the four already-normative inherited Object surfaces with retained ordinary-Protos conformance; C also closes BUG004's root-Object-close versus I/O-Closable compatibility defect. I031's last implementation-bearing slice is D at `0.2.342-SNAPSHOT`, leaving the Core native boundary at `140 sites / 36 providers` at that I031 endpoint. E records final cross-slice closure only; LM008-C/D resume under their own audit owners. No new semantics are introduced. |
...
| I047 | Strict `String.concat` semantics | CLOSED | `578f693b19c7daf739bef69bfbc3d3e1f9abb1b5` | `0.3.43-SNAPSHOT`; D141 Candidate M1 / spec `0.1.424`; focused concat and existing `+` conformance PASS; Protos corpus 1303/1303; unrestricted Maven 1878/1878; top-level publication validation PASS; exact push CI #1893 (`35353428956`) SUCCESS; GitHub #587. |
| I052 | Remove Core fixed-width integer families and preserve interop capability | CLOSED | `ecf563ed01275929d5b85330e8e6259cc85d73d8` | `0.3.56-SNAPSHOT`; D156 Candidate D′ / spec `0.1.430`; removes the eight fixed-width Core numeric families and their Core-only arithmetic/equality/order/hash/conversion institution, preserves reusable width/range host projection as internal interop machinery, reconciles conformance/Test Tool/current documentation, and passes top-level full publication validation. GitHub #628. |
...
| LM008 | Core Language Surface Completeness | CLOSED | `SAME_COMMIT` | LM008-A/B/C/D/E/F are CLOSED. Final F reconciliation cross-checks the retained LM005/LM006 advanced-domain evidence, accounts all LM008 implementation findings under their closed owners, and requires complete Test Tool corpus plus repository publication validation on the closure candidate; no specification, runtime, implementation-version or native-boundary change. |
...
| LM008-B | CLOSED | `e0d804aab36914cef13ecda890c8dbf3a0f1b032` | B1-B4 grammar/evaluation/binding/callable audit complete; retained Protos evidence covers every positive row and no unresolved surface/design classification remains. |
| LM008-C | CLOSED | `ff0e152bf8a1284050f48e65eeecab9719bf6a75` | Object structural/reflection/mutation audit complete; I031/#242 closed the four guest-publication gaps with retained ordinary-Protos regressions. |
| LM008-D | CLOSED | `a775297514808728a5e22d2bc0e7e01f1d74b53c` | Values/Core-collections D1-D4 complete; I032/I034/I035/I036 plus I031 state evidence are closed and every D row is reconciled `COVERED`. |
| LM008-E | CLOSED | `0076a76ab8a441df973931421e107811e6308c41` | E1-E4 control/Error/modules/Prelude audit complete with 51 required Prelude bindings probed, focused intentional-absence evidence and no unresolved E finding. |
| LM008-F | CLOSED | `SAME_COMMIT` | Final LM005/LM006 cross-check, matrix/finding accounting and complete closure validation published; closes parent LM008 without new semantics or implementation. |
...
| TOOL002-I | READY | D076 Candidate D, D077 Candidate D, D087 Candidate B and D091 Candidate A′ are RATIFIED. I1-I6C are CLOSED; I6D is released to implement strict full-TestPlan requirement-to-CaseSpec referential validation and attachment before filtering/sharding. Physical requirements sidecar filename/discovery, catalog CLI/source, exact scope vocabulary, provider APIs, public `std:toml`, a general internal tool package graph and other D077/D087-deferred contracts remain unselected and must cross a later approval gate if reached. |
...
| Item | Description | Status | Closure evidence | Dependencies / notes |
|---|---|---|---|---|
| PERF001 | Core v0.1 baseline benchmark suite | CLOSED | `guillermomolina/protos-benchmarks@45493b49872860f5d29ad3d3a624e0af040c743b` | PERF001-A/B/C/D/E/F/G CLOSED. F retained focused Future/P/Actor timing evidence at `f34e37da11f209aa9f9ea84465822c3362fc4da0` from H3 harness `b8a9eeca85c241f544512a02a6fa29d935f240ef`. G final reproducibility replay/reporting passed from G1 harness `2fad6741b429c3e8d69683806e3aadc64f0812cf` and published at `45493b49872860f5d29ad3d3a624e0af040c743b`: D 44/44, E 18/18, F 12/12 non-retained 2/2/2; retained D/E/F timings remain authoritative and no drift threshold or replacement timing corpus was introduced. |
...
| PERF001-F | CLOSED | `guillermomolina/protos-benchmarks@f34e37da11f209aa9f9ea84465822c3362fc4da0` | Exact H3 harness `b8a9eeca85c241f544512a02a6fa29d935f240ef` measures carrier-fixed Protos `0372a58addc63f305c911811659edd9b2b508420` over the canonical six-workload concurrency corpus; all 12 approved configurations retain 10 startup / 20 warmup / 20 steady reference samples with host/runtime/topology provenance. |
| PERF001-G | CLOSED | `guillermomolina/protos-benchmarks@45493b49872860f5d29ad3d3a624e0af040c743b` | Exact G1 harness `2fad6741b429c3e8d69683806e3aadc64f0812cf` bounded replay: D 44/44 correctness, E 18/18 correctness, F 12/12 configurations at non-retained 2/2/2; baseline report published with retained D/E/F timings as sole authority and no timing-drift threshold. |

Dependency outline: `PERF001-A -> PERF001-B -> PERF001-C -> PERF001-D/E -> PERF001-F -> PERF001-G`; all required PERF001 slices are CLOSED.
...
| DIST004 | Development container OL10 base-OS and OS-tooling migration | CLOSED | DIST004-A `SAME_COMMIT`; DIST004-B `SAME_COMMIT`; DIST004-C `SAME_COMMIT` | Owns the repository migration evaluated by UPSTREAM002 / GitHub #525. Container base `ghcr.io/graalvm/graalvm-community:25i3-25.0.4.1-ol8-20260825` -> `ol10-20260825` with unchanged GraalVM/JDK/Truffle coordinates; `python39` -> image-provided Python 3.12.13; manual checksum-verified Apache Maven 3.9.9 bootstrap -> OS `maven` package (Apache Maven 3.9.9, Red Hat 3.9.9-3) so the exact DIST002 coordinate is retained; `tools/verify_toolchain.py` `devcontainer.maven` binding re-encoded to the OS-package provisioning model. See `docs/project/work/DIST004/DIST004_OL10_CONTAINER_TOOLING_MIGRATION.md`. DIST004-C closes with the complete B5 cross-slice validation in the OL10 container; DIST004 CLOSED. |
| DIST004-A | OL8 -> OL10 container base migration | CLOSED | `21b9ce14` (with `f657f4bd`, `033bbed8`) | Owner-executed container migration published before formal allocation: OL10 image, `python39` replaced by image-provided Python 3.12.13, node provisioning removed after the VS Code extension moved to its dedicated repository. GraalVM/JDK/Truffle coordinates unchanged. |
| DIST004-B | Maven OS-package provisioning contract reconciliation | CLOSED | `SAME_COMMIT` | Exact Maven coordinate remains 3.9.9 (the OL10 OS package is Apache Maven 3.9.9, Red Hat 3.9.9-3). `tools/verify_toolchain.py` development scope re-encoded to check the OS-package provisioning model; verifier unit tests updated and passing; DIST002 record post-closure amendment published; full 1950-test Maven suite in the OL10 container passes with 0 failures/errors/skipped. All-surface CI bindings keep the pre-existing GITHUB017/TOOL005 suspension drift recorded in DIST003 evidence. |
| DIST004-C | Portable-distribution validation and closure | CLOSED | `SAME_COMMIT` | `mvn package -DskipTests` BUILD SUCCESS and `DIST_BUILD: PASS` produce `protos-0.3.0-SNAPSHOT-posix-jvm.zip`; the complete B5 cross-slice gate (`dist/validate_portable.sh`) passes B2 clean-source/archive identity (1770 files), B3 caller-CWD + Package Tool, B4A bundled Test Tool, and B4B exact GraalVM JDK 25.0.4.1 / Truffle 25.3.4.1 `HotSpotTruffleRuntime` with the archive unchanged (sha256 `dc0a459431e22163a660e285160d22ba157f16d735f435b9d7cbfb0fbb82ed28`). Distribution CI reactivation validation remains owned by GITHUB017/TOOL005. |
