...
| `ProtosStandardTcpConnectionProtocol.java` | 7 | resource/capability bridge | D052 requires one hidden shared TcpConnection family protocol whose inherited `read`, `write`, `close`, `shutdownRead`, and `shutdownWrite` behavior validates actual live-family receivers and crosses the host-neutral Byte I/O/lifecycle boundary. PLAT003 installs these Closures once per standard protocol prototype and routes each live resource through independent read/write progress lanes sharing one Closable lifecycle; no per-connection Closure duplication or concrete network backend is selected. |
...
Total audited Core production construction sites: **130 across 33 providers**.
