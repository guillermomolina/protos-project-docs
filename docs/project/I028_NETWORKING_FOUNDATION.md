...
- **E — production backend portability/scalability — CLOSED:** PLAT006/PLAT007/PLAT009 are RATIFIED and E1-E5 are CLOSED; I028-F is READY.
...
Closed at implementation version `0.2.323-SNAPSHOT` under D047/D052 and ratified
PLAT002/PLAT003/PLAT006/PLAT007/PLAT009. No new language semantics, Network grant
policy or durable backend architecture is selected.

`ProtosPolyglotRuntimeHost` now owns a lazy production NIO Network plane. The
plane is created only when an embedding explicitly calls host Network provisioning;
ordinary RuntimeHosts that never request Network create no networking poller or
network authority. Each provision call returns the existing represented
`ProtosNetworkCapabilityValue` for the supplied Prelude and reuses the same host
I/O plane. B3 remains authoritative for whether that already-provisioned
capability is actually granted to a RootActor/standalone initial activation.
This slice therefore does not auto-grant Network to REPL, `-e`, file execution,
`run`, bundled tools, imported modules or child Actors.

The current host plane contains one NIO poller as the initial operational
cardinality, not a Protos identity or permanent sharding decision. PLAT006 still
permits a bounded poller group, sharding, affinity/migration or a different
readiness/completion/native backend later without changing the provisioning
contract. RuntimeHost close stops that plane and releases every registered
connection/listener channel through the existing E1-E4 cleanup callbacks.

IPv6 host interpretation also remains below the Network authority boundary.
Global/non-link-local numeric addresses preserve their exact 128 bits without an
invented interface identity. Link-local scope is accepted only when an exact
assigned scoped address is unique or exactly one active IPv6-link-local interface
can supply the scope; ambiguity fails closed. For PLAT007 `address: null`
listeners, the provider captures the concrete non-wildcard, non-multicast IPv6
addresses on active host interfaces at acquisition time. No interface cache,
dynamic rebinding or portable interface identity is introduced.

Integrated production-path evidence starts with the explicit RuntimeHost
provisioner, passes that exact B3 capability through standalone bootstrap, acquires
an IPv4-null listener, composes concurrent `accept()` with `connectTcp`, transfers
bytes over the resulting E3 connections, closes the logical resources, and proves
RuntimeHost teardown also closes an otherwise-unclosed listener backend. Existing
E1-E4 tests retain cancellation, late-custody, partial-write, half-close,
multi-accept and IPv6-composite evidence. I028-E is CLOSED and I028-F is READY.
...
Closed at implementation version `0.2.315-SNAPSHOT` under the existing Closable/ReadShutdown/
WriteShutdown contracts and ratified PLAT003/PLAT006/PLAT009. No new language or
platform-architecture choice is introduced.

The NIO TcpConnection backend now maps the already-committed logical directional
cutovers to `SocketChannel.shutdownInput()` and `SocketChannel.shutdownOutput()`
on the owning poller. Read-shutdown retires residual `OP_READ` custody before the
physical input half-close. Write-shutdown reaches the backend only after the
host-neutral output lane has settled every preceding accepted write; a residual
physical write is therefore treated as an internal failure rather than truncated.
Neither half-close changes the opposite readiness lane or closes the channel.

Whole-resource `close()` remains the existing stronger one-shot physical release;
it is not implemented as two half-closes. Integrated evidence also exposed and
corrects a pre-existing generic ByteIo admission-order defect: after a successful
local read shutdown followed by whole close, the shared Closable lifecycle now
wins before the local EOF-like read state, as already required by the normative
Socket lifecycle contract.

Real loopback evidence covers output half-close with peer EOF while input remains
usable, input half-close with pending-read local EOF while output remains usable,
and whole close after read half-close with lifecycle failure for subsequent reads
and writes. Retained E3B/E3C evidence continues to cover independent readiness,
short/EOF/cancelled reads, partial writes and first-effect arbitration.

I028-E3 is CLOSED. E4 subsequently closed; E5 now closes E and releases F.
...
Closed at implementation version `0.2.313-SNAPSHOT` under D047/D052, the existing
`spec/io/BYTE_IO.md` read contract and ratified PLAT003/PLAT006/PLAT009.

The connected NIO backend now admits one physical read request for the independent
TcpConnection read lane. Admission is submitted to the E1 poller; the poller first
attempts a non-blocking `SocketChannel.read`. A positive result is delivered
immediately as a non-empty prefix no larger than the requested maximum, `-1`
delivers EOF, and `0` enables only `OP_READ` until the socket becomes readable.
The implementation-local read chunk bound is deliberately non-portable tuning and
does not turn `maxBytes` into an exact-fill requirement.

Read readiness mutates only the `OP_READ` bit of the existing SelectionKey, so E3C
can add/remove `OP_WRITE` independently on the same full-duplex channel.

Backend cancellation does not close the connection. If cancellation wins after a
physical read already returned bytes, the existing ByteIoFlow failed-commit path
re-buffers those bytes at the logical head instead of losing them. Whole connection
close retires any residual started read so the logical lane cannot remain stranded.

Focused loopback evidence covers a pending short read, data already available
before read admission, remote EOF, cancellation followed by a successful next read,
and whole-resource close while a read is pending.

E3B adds no output progress, write buffering, directional half-close, listener,
production Network provisioning, poller cardinality/sharding/affinity or native
backend. E3C is READY.
