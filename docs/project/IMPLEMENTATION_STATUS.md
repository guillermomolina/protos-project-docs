...
| I023 | Standard `while` protocol | IN_PROGRESS | I023-A + I023-B1 published | D044 / spec `0.1.381` satisfies B007; post-A longevity audit subdivides B into bounded replay retention (B1) and adversarial synchronous control (B2), followed by C/D |
...
| I023-B | IN_PROGRESS | — | I023-B1 published | Post-A longevity audit subdivides B so replay-space correctness is established before adversarial synchronous closure; B closes only after B2. |
...
| I023-B2 | READY | — | — | After B1, adversarial synchronous control closure: non-Boolean condition Error timing/freshness, body/condition arity failures at actual activation, Error and non-local-return propagation, exact no-extra-activation/effect behavior, and Future-as-condition/body normal-value boundaries. |
...
| Item | Description | Status | Closure evidence | Dependencies / notes |
|---|---|---|---|---|
| TOOL001 | Package Tool | IN_PROGRESS | `docs/project/TOOL001_PACKAGE_TOOL.md` | Foundations D/E are CLOSED; TOOL001-F physical lockfile v1 has closed F1A/F1B canonical grammar and F1C in-memory parser/writer + round-trip conformance READY. |
...
| TOOL001-F | canonical physical lockfile v1 | IN_PROGRESS | TOOL001-F1A/F1B published | Canonical header/body grammar CLOSED; F1C pure in-memory parser/writer + round-trip conformance READY. |
...
| TOOL001-F1C | canonical lock parser/writer + round-trip conformance | READY | — | Pure bundled-Protos canonical lock-format-1 parser/model/writer; reject non-canonical/structurally invalid input and prove byte-identical round trips. No I/O/resolution/network. |
