...
| I023-B2D | IN_PROGRESS | — | I023-B2D1 published | Body-Future/structured-ownership closure is subdivided into the closed Future-result boundary (B2D1) and READY structured-ownership/cross-B2 reconciliation (B2D2); B2D closes only after B2D2. |
...
| I023-B2D2 | READY | — | — | Reconcile ordinary structured ownership independently of body-result semantics, run cross-B2 evidence, close I023-B2D/B2/B, and make I023-C READY. |
...
| TOOL002 | Test Tool | IN_PROGRESS | `docs/project/TOOL002_TEST_TOOL.md` | A/B/C/D CLOSED; TOOL002-E remains IN_PROGRESS with E1A/E1B/E2A CLOSED and E2B IN_PROGRESS through E2B1; E2B2 READY, then E3/E4 and later F-J. |
...
| TOOL001-F | canonical physical lockfile v1 | IN_PROGRESS | TOOL001-F1/F2A/F2B published | Canonical format, confined I/O and semantic resolution-input digest/stale comparison CLOSED; later consumption/update integration requires fresh audit. |
...
| TOOL001-F2 | physical lock integration | IN_PROGRESS | TOOL001-F2A/F2B published | Confined lock I/O plus canonical semantic digest/stale detection CLOSED; normal-execution lock consumption and explicit resolve/update policy remain unallocated. |
...
| TOOL002-E | IN_PROGRESS | E1A/E1B/E2A CLOSED; E2B IN_PROGRESS through E2B1 with E2B2 READY, followed by E3 Java ownership cutover and E4 final closure. |
...
| TOOL002-E2B | IN_PROGRESS | E2B1 canonical Package/TOML generic-Error CaseSpec normalization CLOSED; E2B2 full-plan execution/full-corpus evidence READY. |
...
| TOOL002-E2B2 | READY | Execute the complete retained Package/TOML plan through bundled Protos `Runner` with `packageExecution`; require selected==plan cases, skipped==0 and all passed; close E2B and release E3. |
| TOOL002-E3 | BLOCKED_BY_DEPENDENCIES | TOOL002-E2B; retire Java ownership of TOML manifest parsing, expectation interpretation and direct fixture execution after Protos owns full-corpus execution. |
| TOOL002-E4 | BLOCKED_BY_DEPENDENCIES | TOOL002-E3; final TOOL002-E ownership/conformance/status reconciliation and transition TOOL002-F to READY. |
...
| DIST001-B | Extracted-distribution smoke/conformance | IN_PROGRESS | DIST001-B1/B2/B3 published | B is subdivided into B1 hygiene/planning, B2 clean-source archive identity, B3 outside-checkout CWD/package execution, B4 bundled Test Tool + optimizing-runtime probe, and B5 cross-slice closure. |
...
| DIST001-B4 | Bundled Test Tool and optimizing-runtime probe | READY | — | After B3, exercise the bundled Test Tool and require exact `com.oracle.truffle.runtime.hotspot.HotSpotTruffleRuntime` from the distribution classpath while preserving the selected JDK22 support boundary. |
...
B1..B5 decomposition above. DIST001-B1/B2/B3 are closed and DIST001-B4 is the
next READY slice. Parent B does not close until B5 publishes the cross-slice
evidence.
