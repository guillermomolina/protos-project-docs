...
| I023 | Standard `while` protocol | IN_PROGRESS | I023-A + I023-B1 + I023-B2A published | D044 / spec `0.1.381` satisfies B007; post-A longevity audit subdivides B into bounded replay retention (B1) and adversarial synchronous control (B2), followed by C/D |
...
| I023-B | IN_PROGRESS | — | I023-B1 + I023-B2A published | Post-A longevity audit subdivides B so replay-space correctness is established before adversarial synchronous closure; B closes only after B2. |
...
| I023-B2 | IN_PROGRESS | — | I023-B2A published | B2 is subdivided into invalid normal condition results (B2A), callback activation timing (B2B), synchronous Error/non-local-return transfer (B2C), and body-Future/structured-ownership closure (B2D); B2 closes only after B2D. |
...
| I023-B2B | READY | — | — | Callback activation contract: condition/body declared-arity failures occur only at the actual zero-argument activation reached by the algorithm; no parameter/default preflight, and an unreachable invalid body activation must not fail. |
| I023-B2C | BLOCKED_BY_DEPENDENCIES | — | — | After B2B, synchronous control transfer: exact Error identity and non-local return from condition/body propagate unchanged; no later callback activation/effect occurs and prior effects remain. |
| I023-B2D | BLOCKED_BY_DEPENDENCIES | — | — | After B2C, prove a normal Future body result is ignored without await/adopt/flatten/cancel, reconcile ordinary structured ownership, run cross-B2 evidence, close B2 and B, and make I023-C READY. |
...
| TOOL001-F | canonical physical lockfile v1 | IN_PROGRESS | TOOL001-F1A/F1B/F1C published | Grammar plus pure in-memory parser/model/writer/canonical rejection CLOSED; next integration slice not yet allocated. |
