...
| TOOL001-F2E2 | verified read-only package-store binding | READY | I024/B009 CLOSED | Implement the exact selected-store-root capture -> ContentIdentity verification -> same captured Filesystem handoff; no package-specific Java/NIO walker. |
...
| DIST002 | Development/release toolchain alignment | IN_PROGRESS | DIST002-A `SAME_COMMIT` | Deferred follow-up, intentionally not a dependency of the frozen DIST001 0.2.236 candidate. Centralize Java bytecode/GraalVM/JDK/Truffle/Maven coordinates; align devcontainer + ordinary CI + release validation; pre-provision the primary runtime so normal gates do not download JDKs on demand; make future runtime migrations explicit. See `docs/project/DIST002_TOOLCHAIN_ALIGNMENT.md`. DIST002-A CLOSED after explicit owner selection: root `toolchain.json` is the canonical Java21/GraalVM25.3.4.1-JDK25.0.4.1/Truffle25.3.4.1/Maven3.9.9 contract; DIST002-B READY. See `docs/project/DIST002_TOOLCHAIN_ALIGNMENT.md`. |
...
| DIST002-B | Align devcontainer + ordinary CI primary runtime | READY | — | DIST002-A is CLOSED. Make development and ordinary CI consume/check the canonical primary runtime and exact Maven coordinate; normal gates pre-provision rather than fetch a JDK on demand. |
| DIST002-C | Align distribution/runtime metadata + validation | BLOCKED_BY_DEPENDENCIES | — | Depends on DIST002-B; migrate DIST001-derived JDK22/Truffle24 live distribution bindings to the canonical contract and remove ordinary on-demand-JDK gate dependence while retaining historical release evidence. |
| DIST002-D | Cross-environment conformance + closure | BLOCKED_BY_DEPENDENCIES | — | Depends on DIST002-C; prove zero drift across development, CI and distribution validation, reconcile documentation/status and close DIST002. |
