...
| I023 | Standard `while` protocol | IN_PROGRESS | I023-A + I023-B1 + I023-B2A + I023-B2B + I023-B2C published + I023-B2D1 published | D044 / spec `0.1.381` satisfies B007; post-A longevity audit subdivides B into bounded replay retention (B1) and adversarial synchronous control (B2), followed by C/D |
...
| I023-B | IN_PROGRESS | — | I023-B1 + I023-B2A + I023-B2B + I023-B2C published + I023-B2D1 published | Post-A longevity audit subdivides B so replay-space correctness is established before adversarial synchronous closure; B closes only after B2. |
...
| I023-B2 | IN_PROGRESS | — | I023-B2A + I023-B2B + I023-B2C published + I023-B2D1 published | B2 is subdivided into invalid normal condition results (B2A), callback activation timing (B2B), synchronous Error/non-local-return transfer (B2C), and body-Future/structured-ownership closure (B2D); B2 closes only after B2D. |
...
| I023-B2D | IN_PROGRESS | — | I023-B2D1 published; B2D2 blocked by B008 | Body-Future/structured-ownership closure retains the closed Future-result boundary (B2D1), while structured ownership/cross-B2 reconciliation (B2D2) is normatively BLOCKED by B008; B2D closes only after B008 is resolved and B2D2 is published. |
...
| I023-B2D2 | BLOCKED | — | B008 | Structured-ownership/cross-B2 reconciliation is blocked until the normative specification uniquely defines the lifetime/ownership consequence when task-backed Future-producing work escapes as the exact normal result of its creating synchronous activation. D044 still requires `while` itself to ignore a body Future result without implicit await/adopt/flatten/cancel; no runtime change is authorized by this blocker record. |
| I023-C | BLOCKED_BY_DEPENDENCIES | — | — | After B2 closes B, suspension/replay and cooperative cancellation: condition/body `Future.value()` suspension resumes the same logical iteration without duplicate effects; no hidden loop checkpoint; cancellation/unwind/`ensure`/structured-child interactions match D044 and existing I022/I009 machinery. |
...
- B008 returned-Future structured-ownership boundary — BLOCKED; required only for I023-B2D2 and downstream I023 closure.
...
| Item | Description | Status | Closure evidence | Dependencies / notes |
|---|---|---|---|---|
| DIST001 | End-user distribution and release engineering | IN_PROGRESS | DIST001-A/B/C + DIST001-D1 + DIST001-D2A published | DIST001-D2B must observe a repaired green snapshot artifact before D closes; E remains dependent on D plus an explicit release decision. |
...
| DIST001-D | CI snapshot artifact | IN_PROGRESS | DIST001-D1 + DIST001-D2A published | First D1 run was green and uploaded the expected artifact, but D2 found an absolute-path external checksum; D2A repairs portability and D2B remains READY for observed repaired-artifact closure. |
...
| DIST001-D2 | Observed CI snapshot artifact closure | IN_PROGRESS | DIST001-D2A published | Run 34100296135 on dc64fb8c4103b44020cd1b718d6850028f08b3a5 was green and artifact 10010254675 existed, but its checksum recorded the runner absolute path. D2A repairs this; D2B must observe a repaired artifact before D closes. |
...
| DIST001-D2B | Observed repaired CI snapshot artifact closure | READY | — | Inspect a real D2A-or-later Distribution snapshot run and downloaded artifact; require green job, exact source SHA, expected artifact name, ZIP + `.sha256`, relative/basename checksum target, and successful `sha256sum -c` before closing D2/D. |
...
DIST001-D1 is closed; the first observed D1 run exposed a non-portable absolute
path in the external checksum; DIST001-D2A repairs it and DIST001-D2B is READY
for observed repaired-artifact closure. DIST001-E remains dependency-gated and
still requires an explicit release decision after D.
