...
| I026 | Truffle tooling foundation | IN_PROGRESS | — | AUD002 hybrid Truffle-first architecture. I026-A was refined before executable work into A1-A4 so the current direct CLI/compiler path is not treated as a compatibility constraint. A1 is published; A2 and D are READY. See docs/project/I026_TRUFFLE_TOOLING_FOUNDATION.md. |
...
| I026-A2 | Canonical Truffle Source compilation boundary | READY | — | I026-A1; connect `ParsingRequest.getSource()` to the real frontend/lowering path and associate all produced roots with the active language/source ownership. |
| I026-A3 | Canonical module source identity | BLOCKED_BY_DEPENDENCIES | — | I026-A2; retain module source identity with ModuleKey instead of reducing loaded modules to anonymous Strings. |
| I026-A4 | Polyglot runtime-entry cutover | BLOCKED_BY_DEPENDENCIES | — | I026-A2 + I026-A3; move CLI/REPL/top-level execution to the real language boundary and retire the old direct top-level compile/call architecture. |
...
| TOOL001-F2E2 | verified read-only package-store binding | IN_PROGRESS | F2E2A CLOSED; F2E2B READY | Production captured-tree ContentIdentity verification is closed in A; B now owns exact already-selected root capture plus host custody of the verified capture, while C remains the final same-capture integration gate. |
...
| TOOL001-F2E2B | exact selected-root capture + verified-capture host custody | READY | TOOL001-F2E2A CLOSED | Re-audit current I024 captured-backend and Package Tool Process-lifetime mechanics, then bind one exact already-selected store root, capture once, invoke A and preserve custody of that exact verified capture. No ambient scan, fetch, hash-policy duplication or plan extension. |
