...
| I023 | Standard `while` protocol | IN_PROGRESS | I023-A + I023-B + I023-C1A published | D044 / spec `0.1.381` + D045 / spec `0.1.382`; B closed, C in progress through C1 |
...
| I023-C | IN_PROGRESS | — | I023-C1A published | Suspension/replay closure is subdivided into C1 condition replay, C2 body replay, C3 cooperative cancellation/ensure, and C4 repeated-suspension/bounded-state adversarial closure; C closes only after C4. |
| I023-C1 | IN_PROGRESS | — | I023-C1A published | Condition suspension/replay is split into one real suspension with exact-once pre-suspend effects (C1A) and repeated/subsequent-iteration suspension closure (C1B). |
...
| I023-C1B | READY | — | — | Exercise condition suspension again across subsequent logical iterations/replay checkpoints, proving completed-iteration compaction does not duplicate or skip condition effects before closing C1. |
| I023-C2 | BLOCKED_BY_DEPENDENCIES | — | — | After C1, body `Future.value()` suspension/replay with exact-once effects and same logical iteration. |
| I023-C3 | BLOCKED_BY_DEPENDENCIES | — | — | After C2, cooperative cancellation through `while` plus `ensure`, with no loop-added cancellation boundary or duplicated unwind. |
| I023-C4 | BLOCKED_BY_DEPENDENCIES | — | — | After C3, repeated condition/body suspension and bounded-state adversarial closure; close parent C and make I023-D READY. |
...
| TOOL001-F2D3B | exact workspace package-backed module resolver | IN_PROGRESS | TOOL001-F2D3B1A/F2D3B1B1 published | Canonical key + selected-root/package index CLOSED; B1B2 exact member-directory binding READY. |
| TOOL001-F2D3B1 | package identity + source mechanism | IN_PROGRESS | TOOL001-F2D3B1A/F2D3B1B1 published | Key identity + root/index CLOSED; member and source binding remain. |
...
| TOOL001-F2D3B1B2 | exact member-location directory binding | READY | — | Physical location traversal/confinement only. |
| TOOL001-F2D3B1B3 | logical module -> exact regular `.protos` source | BLOCKED_BY_DEPENDENCIES | TOOL001-F2D3B1B2 | Source-file mapping begins only after member roots are independently bound. |
...
| Item | Description | Status | Closure evidence | Dependencies / notes |
|---|---|---|---|---|
| DIST001 | End-user distribution and release engineering | IN_PROGRESS | DIST001-A/B/C/D + DIST001-E1/E2 + DIST001-E3A/E3B/E3C1 published | DIST001-E3C2 is READY. Baseline/candidate/version remain unselected; no tag, GitHub Release, or release asset is authorized. |
...
| DIST001-E | First selected GitHub pre-release | IN_PROGRESS | DIST001-E1/E2 + DIST001-E3A/E3B/E3C1 published | E3C is subdivided into C1-C3; C1 is closed and C2 is READY. Candidate-specific E4 still requires E3 closure plus explicit user selection. |
...
| DIST001-E3 | Release metadata, assets and validation preparation | IN_PROGRESS | DIST001-E3A/E3B + DIST001-E3C1 published | E3C1 makes B2/B5 release-aware; E3C2 is READY for envelope verification; E3C3 remains dependency-blocked. No candidate/tag/Release selected. |
...
| DIST001-E3C | Candidate validation entry point and E3 closure | IN_PROGRESS | DIST001-E3C1 published | C1 release-aware B2/B5 plumbing is closed; C2 envelope verifier is READY; C3 composition/closure depends on C2. |
...
| DIST001-E3C2 | Independent release-envelope verifier | READY | — | Verify E3B notes/manifest/checksum identity and digests against the candidate archive/source/runtime metadata; no publication. |
| DIST001-E3C3 | Candidate gate composition and E3 closure | BLOCKED_BY_DEPENDENCIES | — | Depends on E3C2; compose release-aware B5 + envelope verifier + candidate/tag/claim guards; close E3 and make E4 ready for explicit candidate selection. |
...
DIST001-A/B/C/D and DIST001-E1/E2 are closed. DIST001-E3 is IN_PROGRESS:
E3A/E3B and E3C1 are closed; E3C2 is READY and E3C3 remains dependency-blocked.
The existing B5 gate now supports explicit release-candidate identity while
preserving development snapshots as its default. No concrete release baseline,
candidate, public version, tag, GitHub Release, or release asset is selected or
authorized.
