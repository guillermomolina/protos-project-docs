...
| PERF003 | Collection algorithm Truffle compilability | IN_PROGRESS | — | PERF003-A `0.2.170-SNAPSHOT` preserved reduce correctness and cut the valid external `GraphTooBig` count from 40 to 2, but graph size 150500 still exceeded the 150000 limit. Corrective ordinary-Protos reduce traversal refinement is published by this phase at `0.2.175-SNAPSHOT`; an exact external rerun remains required before A can close. PERF003-B remains dependency-gated. |
...
| Slice | Status | Version | Closure evidence | Scope / unblock condition |
|---|---|---|---|---|
| PERF003-A | IN_PROGRESS | `0.2.175-SNAPSHOT` | `CORRECTIVE_IMPLEMENTATION_SAME_COMMIT` | Valid external evidence against `4b2d1c661ed943e51253ec44a324b45e798e1666` shows reduce correctness PASS but 2 remaining `GraphTooBig` failures at graph size 150500 / limit 150000. This corrective phase removes recursive accumulator/result threading while preserving balanced strict left-fold traversal; exact external no-GraphTooBig validation against this new commit is still required. |
