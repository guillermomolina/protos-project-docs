...
| DIST001-B | Extracted-distribution smoke/conformance | IN_PROGRESS | DIST001-B1/B2/B3 + DIST001-B4A published | B1-B3 are closed; B4 is subdivided into B4A Test Tool portability and B4B exact optimizing-runtime evidence; B5 closes the parent after B4. |
...
| DIST001-B4 | Bundled Test Tool and optimizing-runtime probe | IN_PROGRESS | DIST001-B4A published | B4A closes extracted Test Tool portability; B4B remains READY for intact optimizer classpath + exact `HotSpotTruffleRuntime` proof. |
...
| DIST001-B4B | Exact optimizing-runtime probe | READY | — | Preserve the distribution optimizer JARs intact and require the selected runtime evidence to resolve exact `com.oracle.truffle.runtime.hotspot.HotSpotTruffleRuntime`; do not substitute fallback execution. |
| DIST001-B5 | Cross-slice distribution closure | BLOCKED_BY_DEPENDENCIES | — | Depends on DIST001-B4B closing parent B4; then rerun bounded cross-slice evidence, close parent B, and make DIST001-D READY. |
...
B1..B5 decomposition above. DIST001-B1/B2/B3 and B4A are closed; DIST001-B4B is
the next READY slice. Parent B4 remains IN_PROGRESS, and parent B does not close
until B5 publishes the cross-slice evidence.
