...
| TOOL002-D | IN_PROGRESS | TOOL002-D1 CLOSED at `0.2.174-SNAPSHOT`; bootstrap-local `execution(source)` delegates to fresh-Process private capture and returns strict authority-free detached observations. D2 confined corpus read + manifest/TestPlan is READY; D3/D4 remain dependent. `future-*` remains TOOL002-F. |
...
| PERF003 | Collection algorithm Truffle compilability | IN_PROGRESS | — | PERF003-A `0.2.170-SNAPSHOT` cut valid external `array-reduce` `GraphTooBig` failures from 40 to 2. The `0.2.175-SNAPSHOT` correction preserved exact result 528 and reduced graph size from 150500 to 150069 / 150000, leaving 69. A second ordinary-Protos reduce graph refinement is published by this phase at `0.2.178-SNAPSHOT`; an exact external rerun remains required before A can close. PERF003-B remains dependency-gated. |
...
| Slice | Status | Version | Closure evidence | Scope / unblock condition |
|---|---|---|---|---|
| PERF003-A | IN_PROGRESS | `0.2.178-SNAPSHOT` | `SECOND_CORRECTIVE_IMPLEMENTATION_SAME_COMMIT` | Valid external evidence against `eb8b9c588bb363309da5193bf8d98d64d5456cee` shows reduce correctness PASS and 2 remaining `GraphTooBig` failures at graph size 150069 / limit 150000. This second corrective phase caches immutable internal initial/snapshot cardinalities and removes a redundant helper result expression without changing the balanced strict left fold; exact external no-GraphTooBig validation against this new commit is still required. |
